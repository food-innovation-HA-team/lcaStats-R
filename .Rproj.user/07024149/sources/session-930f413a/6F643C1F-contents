  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      sliderInput("completeness", "Completeness",
                  min = 1, max = 5, value = 2, step = 1, ticks = TRUE),
      sliderInput("temporal_correlation", "Temporal correlation",
                  min = 1, max = 5, value = 3, step = 1, ticks = TRUE),
      sliderInput("geographical_correlation", "Geographical correlation",
                  min = 1, max = 5, value = 2, step = 1, ticks = TRUE),
      sliderInput("technology_correlation", "Technology correlation",
                  min = 1, max = 5, value = 3, step = 1, ticks = TRUE),
...
server <- function(input, output, session) {
  ...
  # Reactive: compute uncertainty
  result <- reactive({
    scores_to_gsd2(
      reliability              = input$reliability,
      completeness             = input$completeness,
      temporal_correlation     = input$temporal_correlation,
      geographical_correlation = input$geographical_correlation,
      technology_correlation   = input$technology_correlation,
  ...
  output$metrics_table <- renderTable({
    r <- result()
    data.frame(
      Parameter = c("GSD² (ecoinvent)", "GSD", "σ_ln", "Combined variance (σ²_ln)"),
      Value     = c(
        round(r$gsd2,        4),
        round(r$gsd,         4),
        round(r$sigma_ln,    6),
        round(r$combined_var, 6)
      )
    )
  ...
  output$breakdown_table <- renderTable({
    r        <- result()
    sources  <- c("Basic uncertainty", INDICATOR_LABELS)
